...
<div class="container-fluid">
    <h1>Choose a room</h1>
    <div class="table">
        <form action="roomAction.php" id="room-reserve" method="POST" class="form">
            <table class="table table-responsive">
                ...
                <tr>
                    <td>
                        <img src="../images/regular.jpg" class="roomtype img-responsive">
                        <p class="text-center">Regular Room</p>
                    </td>
                    <td>
                        <p>with terrace and garden set with aircon and TV with toilet and bathroom one (1) double deck bed 6 persons capacity per room</p>
                        <p>Capacity: 6 Persons</p>
                    </td>
                    <td>
                        <p>P2,500.00 + P200 (Entrance fee)</p>
                    </td>
                    <td>
                        <select name="numberofrooms">
                            ...
                            //query
                            $sql = "SELECT COUNT(roomstatus AND roomtype) FROM rooms WHERE roomstatus = 'Available' AND roomtype = 'Regular'";
                            $result = mysqli_query($connect, $sql);
                            die($result->fetch_array());

                            while($row = $result->fetch_array()) {
                                $roomtype = $row['roomtype'];
                                ?>
                                <option disabled selected style="display: none;" value="0" name="zero">0</option>
                                <?php
                                for ($regular = 1; $regular <= $row[0]; $regular++) {
                                    ?>
                                    <option value="<?php echo $regular; ?>"> <?php echo $regular; ?> </option>
                                    <?php
                                }
                            }
                            ?>
                ...
                <tr>
                    <td>
                        <img src="../images/couple.jpg" class="roomtype img-responsive">
                        <p class="text-center">Couple Room</p>
                    </td>
                    <td>
                        <p>with terrace and garden set with aircon and TV with toilet and bathroom one (1) double deck bed 6 persons capacity per room</p>
                        <p>Capacity: 6 Persons</p>
                    ...
                    <td>
                        <select name="numberofrooms">
                            ...
                            include ('dbConnection.php');
                            //query
                            $sql = "SELECT COUNT(roomstatus AND roomtype) FROM rooms WHERE roomstatus = 'Available' AND roomtype = 'Couple'";
                            $result = mysqli_query($connect, $sql);
                            ...
                            while($row = $result->fetch_array()) {
                                ...
                                <option disabled selected style="display: none;" value="0" name="zero">0</option>
                                <?php
                                for ($couple = 1; $couple <= $row[0]; $couple++) {
                                    ?>
                                    <option value="<?php echo $couple; ?>"> <?php echo $couple; ?> </option>
                                    <?php
                                }
                            }
                            ?>
                ...
                <tr>
                    <td colspan="4"><button type="submit" class="btn btn-primary pull-right">Reserve room</button>  </td>
                </tr>
                </tbody>
            </table>
        </form>
...
<script>
    $(function () {
       $('numberofrooms').submit(function () {
           if($('regular').val == '' || $('couple').val == '') {
               alert('Please enter your desired number of rooms to be reserve.')
               return false;
           }
       })
    });
